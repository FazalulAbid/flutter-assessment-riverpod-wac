class SubmittedTimesheetDto {
  final String week;
  final String weekId;
  final List<ShiftDto> shifts;

  SubmittedTimesheetDto({required this.week, required this.weekId, required this.shifts});

  factory SubmittedTimesheetDto.fromJson(Map<String, dynamic> json) {
    return SubmittedTimesheetDto(
      week: json['week'] ?? '',
      weekId: json['week_id'] ?? '',
      shifts: (json['shifts'] as List).map((e) => ShiftDto.fromJson(e)).toList(),
    );
  }
}
